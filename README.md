...
- MetalKit renderer with switchable environments: a shaded 3D workspace room, a target-tunnel scene with near-3D billboard objects, a theater-screen auditorium view with an oversized screen for judging flat-image stability as distance changes, and a scenic window-view environment with a foreground frame over a layered landscape
...
Use the `Scene` control in the inspector to swap between the workspace room, the target tunnel, the theater-screen auditorium environment, and the scenic window-view environment.
